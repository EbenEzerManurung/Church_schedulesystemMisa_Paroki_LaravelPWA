...
@section('title', 'Edit Tugas Pelayanan')
@section('page-title', 'Edit Tugas Pelayanan')

@section('content')
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">
            <i class="fas fa-edit mr-2 text-yellow-500"></i> Edit Tugas Pelayanan
        </h1>
    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        <form action="{{ route('duties.update', $duty) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        <i class="fas fa-barcode mr-1 text-gray-400"></i> Kode Tugas <span class="text-red-500">*</span>
                    </label>
                    <input type="text" 
                           name="code" 
                           value="{{ old('code', $duty->code) }}"
                           class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 bg-gray-50 @error('code') border-red-500 @enderror"
                           readonly>
                    <p class="text-xs text-gray-500 mt-1">
                        <i class="fas fa-lock mr-1"></i> Kode tugas tidak dapat diubah
                    </p>
                    @error('code') 
            ...
            <div class="mt-6">
                ...
                <textarea name="description" 
                          ...
                          placeholder="Jelaskan tugas dan tanggung jawab pelayanan ini...">{{ old('description', $duty->description) }}</textarea>
                @error('description') 
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div>
                    ...
                    <input type="number" 
                           name="min_person" 
                           value="{{ old('min_person', $duty->min_person ?? 1) }}"
                           min="0"
                ...
                <div>
                    ...
                    <input type="number" 
                           name="max_person" 
                           value="{{ old('max_person', $duty->max_person) }}"
                           min="0"
            ...
            <div class="mt-6">
                ...
                <div class="flex space-x-4">
                    <label class="inline-flex items-center">
                        <input type="radio" name="is_active" value="1" class="form-radio text-blue-500" {{ old('is_active', $duty->is_active) ? 'checked' : '' }}>
                        <span class="ml-2 text-sm text-gray-700">
                            <i class="fas fa-check-circle text-green-500 mr-1"></i> Aktif
                        </span>
                    </label>
                    <label class="inline-flex items-center">
                        <input type="radio" name="is_active" value="0" class="form-radio text-red-500" {{ old('is_active', $duty->is_active) ? '' : 'checked' }}>
                        <span class="ml-2 text-sm text-gray-700">
                ...
                <p class="text-xs text-gray-500 mt-1">
                    <i class="fas fa-info-circle mr-1"></i> Status nonaktif akan menyembunyikan tugas dari daftar
                </p>
                @error('is_active') 
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mt-6 p-4 rounded-lg border 
                @if($ketersediaan['status'] == 'membutuhkan_petugas') bg-yellow-50 border-yellow-200
                @elseif($ketersediaan['status'] == 'cukup') bg-green-50 border-green-200
                @else bg-red-50 border-red-200 @endif">
                <div class="flex items-start">
                    <i class="fas fa-info-circle mt-1 mr-3 text-gray-500"></i>
                    <div>
                        <h4 class="text-sm font-semibold text-gray-800">Status Ketersediaan Saat Ini:</h4>
                        <div class="flex items-center mt-1">
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium {{ $ketersediaan['badge'] }}">
                                <i class="fas {{ $ketersediaan['icon'] }} mr-1"></i>
                                {{ $ketersediaan['label'] }}
                            </span>
                            <span class="text-xs text-gray-600 ml-2">
                                ({{ $petugasCount }} petugas terdaftar)
                            </span>
                        </div>
                        <p class="text-xs text-gray-600 mt-1">
                            {{ $ketersediaan['message'] }}
                        </p>
                        <ul class="text-xs text-gray-500 mt-2 space-y-0.5">
                            <li>• <span class="font-medium">Minimum:</span> {{ $duty->min_person }} petugas</li>
                            @if($duty->max_person)
                                <li>• <span class="font-medium">Maksimum:</span> {{ $duty->max_person }} petugas</li>
                            @else
                                <li>• <span class="font-medium">Maksimum:</span> Tidak ada batas</li>
                            @endif
                        </ul>
                    </div>
                </div>
                ...
                <div class="flex items-start">
                    ...
                    <div>
                        ...
                        <ul class="text-xs text-blue-700 mt-1 space-y-1">
                            ...
                            <li>• Status akan otomatis berubah: 
                                <span class="bg-yellow-200 px-1 rounded">Membutuhkan Petugas</span> → 
                                <span class="bg-green-200 px-1 rounded">Cukup</span> → 
                                <span class="bg-red-200 px-1 rounded">Full</span>
                            </li>
                        </ul>
                    </div>
                </div>
            ...
            <div class="flex justify-end space-x-3 mt-6 pt-6 border-t border-gray-200">
                ...
                <button type="submit" class="px-6 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition duration-200">
