...
@section('page-title', 'Detail Ketersediaan')

@section('content')
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">
            <i class="fas fa-info-circle mr-2 text-blue-500"></i> Detail Ketersediaan
        </h1>
        <div class="flex space-x-2">
            @if($assignment->status == 'pending' && (auth()->user()->isAdmin() || $assignment->user_id == auth()->id()))
            <a href="{{ route('availability.edit', $assignment->id) }}" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded-lg transition">
                <i class="fas fa-edit mr-2"></i> Edit Ketersediaan
            </a>
            @endif
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="bg-gray-50 p-4 rounded-lg">
            <label class="text-sm text-gray-500">Hari</label>
            <p class="font-semibold text-gray-800 text-lg">
                {{ $assignment->event_date ? \Carbon\Carbon::parse($assignment->event_date)->translatedFormat('l') : '-' }}
            </p>
        </div>
    </div>
</div>
                        <div class="bg-gray-50 p-4 rounded-lg">
                            ...
                            @if($assignment->duty && $assignment->duty->description)
                                <p class="text-sm text-gray-500 mt-1">{{ $assignment->duty->description }}</p>
                            @endif
                        </div>
                        <div class="bg-gray-50 p-4 rounded-lg">
                            <label class="text-sm text-gray-500">Petugas</label>
                            <p class="font-semibold text-gray-800">{{ $assignment->user->name ?? 'Tidak tersedia' }}</p>
                            <p class="text-sm text-gray-500">{{ $assignment->user->email ?? '-' }}</p>
                        </div>
                        <div class="bg-gray-50 p-4 rounded-lg">
                            <label class="text-sm text-gray-500">Status Penugasan</label>
                            <p>
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg">
                            <label class="text-sm text-gray-500">Status Ketersediaan</label>
                            <p>
                        ...
                        <div class="md:col-span-2 bg-red-50 p-4 rounded-lg border border-red-200">
                            <label class="text-sm text-red-600 font-semibold">
                                <i class="fas fa-info-circle mr-1"></i> Alasan Ketidaksediaan
                            </label>
                            <p class="text-red-700 mt-1">{{ $assignment->unavailable_reason }}</p>
                        </div>
                        @endif
                        @if($assignment->notes)
        ...
        <div>
            ...
            <div class="bg-white rounded-lg shadow-lg overflow-hidden mt-4">
                ...
                <div class="p-6">
                    <div class="space-y-3">
                        <div>
                            <label class="text-sm text-gray-500">ID Penugasan</label>
                            <p class="text-gray-800 font-mono text-sm">#{{ $assignment->id }}</p>
                        </div>
                        <div>
                            <label class="text-sm text-gray-500">Status</label>
                            <p>
                                <span class="px-2 py-1 rounded-full text-xs font-semibold {{ $assignment->status_badge }}">
                                    {{ $assignment->status_label }}
                                </span>
                            </p>
                        </div>
                        ...
                        <div>
                            <label class="text-sm text-gray-500">Waktu Ibadah</label>
                            <p class="text-gray-800">{{ \Carbon\Carbon::parse($assignment->schedule->time)->format('H:i') }}</p>
                        </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="bg-white rounded-lg shadow-lg overflow-hidden mt-4">
                ...
                <div class="p-6">
                    <div class="space-y-2">
                        @if(isset($liturgi) && $liturgi)
                        <a href="{{ route('kalender-liturgi.show', $liturgi->id) }}" class="block w-full text-center bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg transition duration-200">
                            <i class="fas fa-church mr-2"></i> Lihat Kalender Liturgi
                        </a>
                        @endif
                        @if(auth()->user()->isAdmin())
                        <a href="{{ route('availability.index') }}?search={{ $assignment->user->name ?? '' }}" class="block w-full text-center bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg transition duration-200">
                            <i class="fas fa-users mr-2"></i> Lihat Semua Tugas {{ $assignment->user->name ?? '' }}
                        </a>
                        @endif
                    </div>
                </div>
